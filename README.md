...
- **缺失歌词/翻译的报错占位**：兜底链路最终未命中时，歌曲区域直接显示“通用插件歌词匹配失败 / 通用插件翻译匹配失败”报错文案，不再回退展示“歌名 - 歌手”；同一首歌的重复占位回调不会覆盖报错。
- **兜底链路分流**：已安装内置插件/独立模块的播放器，专用插件未命中 → 通用插件（LRCLIB）兜底 → 仍未命中直接报错，不再继续走四平台取词；完全未安装任何插件的播放器，通用插件（LRCLIB）未命中后继续走四平台在线源整首匹配歌词与翻译，仍失败才报错。
- **二次匹配**：匹配不准或未命中时，可在“在线翻译源”页面手动输入歌名（必填）、歌手（必填）、专辑（选填），向已启用来源搜索候选歌曲列表；结果会按标题、歌手、专辑的相关性本地排序，并过滤掉与歌名和歌手均无关的候选。选中正确候选后重新抓取该来源歌词，并自动向四平台补齐翻译后重新发布。
...
### 6. 超级岛摘要态封面背景与自适应文字颜色

- **摘要态封面背景样式**：超级岛折叠态（摘要胶囊）支持「圆形封面 / 旋转圆形封面 / 渐变封面 / 线性渐变 / 应用图标」等音频封面样式；其中「线性渐变」会把专辑封面整幅铺入胶囊、叠加同源渐变压暗，贴近小米焦点通知卡片的线性渐变观感。
- **摘要态歌词位置与样式**：支持左侧/右侧歌词显示，并对主句、伴唱、翻译、下一句等内容的字体大小、字重、斜体、颜色分别配置。
- **自适应文字颜色（可读性救援）**：当摘要态背景为「渐变封面 / 线性渐变」时，背景取自封面、颜色不可预知。此时会依据所选字体颜色（默认 / 莫奈取色 / 封面色 / 封面渐变色 / 自定义）与封面背景的实际对比度自动救援——对比不足时把文字亮度沿背景明暗的反方向持续推到对端（最远接近纯白/纯黑），直至达到目标对比度，避免出现「粉字压粉底」这类歌词看不清的情况。
- **展开态恢复原生短胶囊**：点开超级岛大窗（展开态）时，顶部摘要胶囊会恢复为系统原生短胶囊长度（与视频小窗时一致），不再保留歌词驱动的动态长胶囊；收起后再自动恢复歌词长胶囊。
