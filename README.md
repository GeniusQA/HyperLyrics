...
- **通用兜底（内置 LRCLIB，无需 Provider 插件）**：对未安装专用 Provider 插件/独立模块的三方播放器（如酷狗概念版），不依赖任何插件即可获取歌词——从标准 MediaSession 读取歌名/歌手/时长，交由内置兜底 Provider（LRCLIB）在线匹配，命中后整首注入歌词与译文。
- **通用兜底触发与约束**：`activeProviderPackageName == null` 即视为通用兜底播放器，绕过单 App 在线开关，保证「所有播放器都能匹配歌词」；前提是播放器通过 MediaSession 暴露非空 TITLE 元数据，否则兜底无法启动。
- **兜底歌词上岛前提**：通用兜底歌词必须经桥把歌词归属注册到对应播放器包名（`LyriconDataBridge.currentLyricPackageName`），否则超级岛渲染器因包名为空而跳过，导致兜底歌词无法显示到岛（已修复：兜底发布时统一注册包名）。
