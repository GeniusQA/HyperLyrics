...
- **超级岛·摘要态歌词**：摘要态仍保持默认最多显示两行；当当前行存在翻译/发音/伴唱时，优先显示这些附加内容，不占用下一句预览位置。
...
- **多行歌词与高度上限**：以「歌词区域高度上限」(120~500dp) 取代固定行数，系统按当前字号反推可显示的最大行数（贴近 LyricFocus 的高度模型）；可在 AOD、锁屏歌词、通知中心歌词、自定义 AOD 与超级岛展开态中分别配置；开启多行歌词后，当前行存在翻译/发音/伴唱时优先显示附加内容，不显示下一句歌词。
